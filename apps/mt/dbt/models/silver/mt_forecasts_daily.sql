...
select
    ...
    cast(temperature_maximum as float)       as temperature_max_c,
    cast(temperature_minimum as float)       as temperature_min_c,
...
from {{ ref('stg_mt_forecasts_daily') }}

{% if is_incremental() %}
where (cast(forecast_timestamp as timestamp) at time zone 'Europe/Rome') > (
    select coalesce(max(forecast_at), '1900-01-01'::timestamptz) - interval '7 days'
    from {{ this }}
)
{% endif %}
